...
object MimeUtils {
    private val mimeTypeToExtensionMap: MutableMap<String, String> = mutableMapOf()
    private val extensionToMimeTypeMap: MutableMap<String, String> = mutableMapOf()
    ...
    private fun getContentTypesPropertiesStream(): InputStream? {
        // User override?
        ...
        val f = File(
            ...
            "lib" + File.separator.toString() + "content-types.properties"
        )
        if (f.exists()) {
            try {
                return FileInputStream(f)
    ...
    private fun applyOverrides() {
        ...
        val stream: InputStream = getContentTypesPropertiesStream() ?: return
        try {
            try {
