...
import com.s16.drawable.CircularProgressDrawable
...
class MaterialProgressBar: ProgressBar {
    ...
    private fun initialize(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ...
            when(baseSize) {
                largeSize -> {
                    progressDrawable.setCenterRadius(dpToPixel(context, CENTER_RADIUS_LARGE))
                    progressDrawable.setStrokeWidth(dpToPixel(context, STROKE_WIDTH_LARGE))
                }
                smallSize -> {
                    progressDrawable.setCenterRadius(dpToPixel(context, CENTER_RADIUS_SMALL))
                    progressDrawable.setStrokeWidth(dpToPixel(context, STROKE_WIDTH_SMALL))
                }
                else -> {
                    progressDrawable.setCenterRadius(dpToPixel(context, CENTER_RADIUS))
                    progressDrawable.setStrokeWidth(dpToPixel(context, STROKE_WIDTH))
                }
            }
